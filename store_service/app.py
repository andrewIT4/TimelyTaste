...
@app.route('/store_api/stores/<store_id>', methods=['GET'])
...
@app.route('/store_api/stores', methods=['POST'])
def create_store():
    try:
        data = request.json
...
@app.route('/store_api/stores/<store_id>', methods=['PUT'])
def update_store(store_id):
    try:
        ...
        return jsonify({'message': 'Store id is missing'}), 422
    except:
        return jsonify({'message': 'No json body received'}), 400


@app.route('/store_api/stores/<store_id>', methods=['DELETE'])
